...
def showHandJoints(imgIn, gtIn, estIn=None, filename=None, upscale=1, lineThickness=2):
    ...
    jointConns = [[0, 1,2,3, 17], [0, 4,5,6,18], [0, 7,8,9, 20],[0, 10,11,12,19], [0, 13, 14, 15, 16]]
...
def fig2img(fig):
    ...
    buf = fig2data(fig)
    w, h, d = buf.shape
    return np.array(Image.frombytes("RGBA", (w, h), buf.tostring()), np.uint8)



def get_default_smpl_params():
    pose = np.array(
        [0.603789, -0.367011, 0.453897, 0.651877, -0.656605, -0.212550, -0.632679, 0.205984, 1.017134,
         -0.031939, 0.332727, -0.415808, -1.454459, 1.093990, -0.064683, 0.319805, 0.742956, 1.634443,
         0.273575, 0.269810, 0.660156, -0.086993, -1.947417, -0.632760, -0.609980, 1.683600, 2.866103,
         2.875561, -2.064497, -0.469832, -1.280301, -0.474019, -3.393826, -0.557894, -0.295235, 1.362216,
         -1.732486, 0.374566, -0.569821, 0.691290, 0.227309, -1.397949, 2.431975, 0.010688, 1.306405],
        dtype=np.float)
    # quaternion rotation
    q = np.array([1.479012, - 1.948220, 0.975287, - 0.935092])
    # tranlation
    camq = np.array([8.316971, 0.449894, 0.585891, 0.008886])
    return pose, q, camq
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base = False):
    ...
    if rotate_base:
        ...
        rot_x = Variable(torch.from_numpy(np_rot_x).float()).cuda(device=DEVICENUM)
    ...
    def make_A(R, t):
        ...
        t_homo = torch.cat([t, Variable(torch.ones(N, 1, 1)).cuda(device=DEVICENUM)], dim = 1)
    ...
    #aa = np.array([15, 3, 6, 12, 9])
    #bb = np.array([16, 17, 18, 19, 20])

    #for i in range(5):
    #    j_here = Js[:,aa[i]] - Js[:,bb[i]]
    #    A_here =  make_A(Rs[:,i], j_here)
    #    res_here = torch.matmul(results[aa[i]], A_here)
    #    results.append(res_here)

    results = torch.stack(results, dim = 1)

    new_J = results[:, :, :3, 3]
    Js_w0 = torch.cat([Js, Variable(torch.zeros(N, 16, 1, 1)).cuda(device=DEVICENUM)], dim = 2)
