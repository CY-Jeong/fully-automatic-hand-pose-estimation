...
class LinearModel(nn.Module):
    '''
        input param:
            fc_layers: a list of neuron count, such as [N, 1024, 1024, 85]
    ...
    '''
    def __init__(self, fc_layers, fc_layers2, use_dropout, drop_prob, use_ac_func):
        super(LinearModel, self).__init__()
        self.fc_layers     = fc_layers
        self.fc_layers2    = fc_layers2
    ...
    def create_layers(self):
        l_fc_layer = len(self.fc_layers)
        l_fc_layer2 = len(self.fc_layers2)
        l_use_drop = len(self.use_dropout)
        ...
        self.fc_blocks2 = nn.Sequential()
        for _ in range(l_fc_layer2 - 1):
            self.fc_blocks2.add_module(
                name='regressor_fc_{}'.format(_),
                module=nn.Linear(in_features=self.fc_layers2[_], out_features=self.fc_layers2[_ + 1])
            )
