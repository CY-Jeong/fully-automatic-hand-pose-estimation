    ...
    def skel_fit(self, points_3d):
        ...
        for ix in tqdm.tqdm(range(iteration)):
            for view in range(cfg.VIEW_NUM):
                ...
                loss.backward()
                optimizer.step()
                if view == 0 and ix%50==0:
                    self.pltshow(skk2d, points_3d, ix)
            print(f"epoch : {ix} loss : {loss}")
    def pltshow(self, skk2d, point3d, ix):
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(xs=point3d[:, self.skkidx, 0].detach().cpu().numpy(),
                   ys=point3d[:, self.skkidx, 1].detach().cpu().numpy(),
                   zs=point3d[:, self.skkidx, 2].detach().cpu().numpy(), c='r', marker='o')
        ax.plot([point3d[:, 0, 0], point3d[:, 4, 0]],
                [point3d[:, 0, 1], point3d[:, 4, 1]],
                zs=[point3d[:, 0, 2], point3d[:, 4, 2]], c='blue')
        ax.plot([point3d[:, 3, 0], point3d[:, 4, 0]],
                [point3d[:, 3, 1], point3d[:, 4, 1]],
                zs=[point3d[:, 3, 2], point3d[:, 4, 2]], c='blue')
        ax.plot([point3d[:, 2, 0], point3d[:, 3, 0]],
                [point3d[:, 2, 1], point3d[:, 3, 1]],
                zs=[point3d[:, 2, 2], point3d[:, 3, 2]], c='blue')
        ax.plot([point3d[:, 1, 0], point3d[:, 2, 0]],
                [point3d[:, 1, 1], point3d[:, 2, 1]],
                zs=[point3d[:, 1, 2], point3d[:, 2, 2]], c='blue')
        ax.plot([point3d[:, 0, 0], point3d[:, 8, 0]],
                [point3d[:, 0, 1], point3d[:, 8, 1]],
                zs=[point3d[:, 0, 2], point3d[:, 8, 2]])
        ax.plot([point3d[:, 7, 0], point3d[:, 8, 0]],
                [point3d[:, 7, 1], point3d[:, 8, 1]],
                zs=[point3d[:, 7, 2], point3d[:, 8, 2]])
        ax.plot([point3d[:, 6, 0], point3d[:, 7, 0]],
                [point3d[:, 6, 1], point3d[:, 7, 1]],
                zs=[point3d[:, 6, 2], point3d[:, 7, 2]])
        ax.plot([point3d[:, 5, 0], point3d[:, 6, 0]],
                [point3d[:, 5, 1], point3d[:, 6, 1]],
                zs=[point3d[:, 5, 2], point3d[:, 6, 2]])
        ax.plot([point3d[:, 0, 0], point3d[:, 12, 0]],
                [point3d[:, 0, 1], point3d[:, 12, 1]],
                zs=[point3d[:, 0, 2], point3d[:, 12, 2]])
        ax.plot([point3d[:, 11, 0], point3d[:, 12, 0]],
                [point3d[:, 11, 1], point3d[:, 12, 1]],
                zs=[point3d[:, 11, 2], point3d[:, 12, 2]])
        ax.plot([point3d[:, 10, 0], point3d[:, 11, 0]],
                [point3d[:, 10, 1], point3d[:, 11, 1]],
                zs=[point3d[:, 10, 2], point3d[:, 11, 2]])
        ax.plot([point3d[:, 9, 0], point3d[:, 10, 0]],
                [point3d[:, 9, 1], point3d[:, 10, 1]],
                zs=[point3d[:, 9, 2], point3d[:, 10, 2]])
        ax.plot([point3d[:, 0, 0], point3d[:, 16, 0]],
                [point3d[:, 0, 1], point3d[:, 16, 1]],
                zs=[point3d[:, 0, 2], point3d[:, 16, 2]])
        ax.plot([point3d[:, 15, 0], point3d[:, 16, 0]],
                [point3d[:, 15, 1], point3d[:, 16, 1]],
                zs=[point3d[:, 15, 2], point3d[:, 16, 2]])
        ax.plot([point3d[:, 14, 0], point3d[:, 15, 0]],
                [point3d[:, 14, 1], point3d[:, 15, 1]],
                zs=[point3d[:, 14, 2], point3d[:, 15, 2]])
        ax.plot([point3d[:, 13, 0], point3d[:, 14, 0]],
                [point3d[:, 13, 1], point3d[:, 14, 1]],
                zs=[point3d[:, 13, 2], point3d[:, 14, 2]])
        ax.plot([point3d[:, 0, 0], point3d[:, 20, 0]],
                [point3d[:, 0, 1], point3d[:, 20, 1]],
                zs=[point3d[:, 0, 2], point3d[:, 20, 2]], c='red')
        ax.plot([point3d[:, 19, 0], point3d[:, 20, 0]],
                [point3d[:, 19, 1], point3d[:, 20, 1]],
                zs=[point3d[:, 19, 2], point3d[:, 20, 2]], c='red')
        ax.plot([point3d[:, 18, 0], point3d[:, 19, 0]],
                [point3d[:, 18, 1], point3d[:, 19, 1]],
                zs=[point3d[:, 18, 2], point3d[:, 19, 2]], c='red')
        ax.plot([point3d[:, 17, 0], point3d[:, 18, 0]],
                [point3d[:, 17, 1], point3d[:, 18, 1]],
                zs=[point3d[:, 17, 2], point3d[:, 18, 2]], c='red')

        skkk2d = skk2d[:, self.skkidx2, :]
        ax.plot([skkk2d[:, 0, 0], skkk2d[:, 4, 0]], [skkk2d[:, 0, 1], skkk2d[:, 4, 1]],
                zs=[skkk2d[:, 0, 2], skkk2d[:, 4, 2]], c='blue')
        ax.plot([skkk2d[:, 3, 0], skkk2d[:, 4, 0]], [skkk2d[:, 3, 1], skkk2d[:, 4, 1]],
                zs=[skkk2d[:, 3, 2], skkk2d[:, 4, 2]], c='blue')
        ax.plot([skkk2d[:, 2, 0], skkk2d[:, 3, 0]], [skkk2d[:, 2, 1], skkk2d[:, 3, 1]],
                zs=[skkk2d[:, 2, 2], skkk2d[:, 3, 2]], c='blue')
        ax.plot([skkk2d[:, 1, 0], skkk2d[:, 2, 0]], [skkk2d[:, 1, 1], skkk2d[:, 2, 1]],
                zs=[skkk2d[:, 1, 2], skkk2d[:, 2, 2]], c='blue')
        ax.plot([skkk2d[:, 0, 0], skkk2d[:, 8, 0]], [skkk2d[:, 0, 1], skkk2d[:, 8, 1]],
                zs=[skkk2d[:, 0, 2], skkk2d[:, 8, 2]])
        ax.plot([skkk2d[:, 7, 0], skkk2d[:, 8, 0]], [skkk2d[:, 7, 1], skkk2d[:, 8, 1]],
                zs=[skkk2d[:, 7, 2], skkk2d[:, 8, 2]])
        ax.plot([skkk2d[:, 6, 0], skkk2d[:, 7, 0]], [skkk2d[:, 6, 1], skkk2d[:, 7, 1]],
                zs=[skkk2d[:, 6, 2], skkk2d[:, 7, 2]])
        ax.plot([skkk2d[:, 5, 0], skkk2d[:, 6, 0]], [skkk2d[:, 5, 1], skkk2d[:, 6, 1]],
                zs=[skkk2d[:, 5, 2], skkk2d[:, 6, 2]])
        ax.plot([skkk2d[:, 0, 0], skkk2d[:, 12, 0]], [skkk2d[:, 0, 1], skkk2d[:, 12, 1]],
                zs=[skkk2d[:, 0, 2], skkk2d[:, 12, 2]])
        ax.plot([skkk2d[:, 11, 0], skkk2d[:, 12, 0]], [skkk2d[:, 11, 1], skkk2d[:, 12, 1]],
                zs=[skkk2d[:, 11, 2], skkk2d[:, 12, 2]])
        ax.plot([skkk2d[:, 10, 0], skkk2d[:, 11, 0]], [skkk2d[:, 10, 1], skkk2d[:, 11, 1]],
                zs=[skkk2d[:, 10, 2], skkk2d[:, 11, 2]])
        ax.plot([skkk2d[:, 9, 0], skkk2d[:, 10, 0]], [skkk2d[:, 9, 1], skkk2d[:, 10, 1]],
                zs=[skkk2d[:, 9, 2], skkk2d[:, 10, 2]])
        ax.plot([skkk2d[:, 0, 0], skkk2d[:, 16, 0]], [skkk2d[:, 0, 1], skkk2d[:, 16, 1]],
                zs=[skkk2d[:, 0, 2], skkk2d[:, 16, 2]])
        ax.plot([skkk2d[:, 15, 0], skkk2d[:, 16, 0]], [skkk2d[:, 15, 1], skkk2d[:, 16, 1]],
                zs=[skkk2d[:, 15, 2], skkk2d[:, 16, 2]])
        ax.plot([skkk2d[:, 14, 0], skkk2d[:, 15, 0]], [skkk2d[:, 14, 1], skkk2d[:, 15, 1]],
                zs=[skkk2d[:, 14, 2], skkk2d[:, 15, 2]])
        ax.plot([skkk2d[:, 13, 0], skkk2d[:, 14, 0]], [skkk2d[:, 13, 1], skkk2d[:, 14, 1]],
                zs=[skkk2d[:, 13, 2], skkk2d[:, 14, 2]])
        ax.plot([skkk2d[:, 0, 0], skkk2d[:, 20, 0]], [skkk2d[:, 0, 1], skkk2d[:, 20, 1]],
                zs=[skkk2d[:, 0, 2], skkk2d[:, 20, 2]], c='red')
        ax.plot([skkk2d[:, 19, 0], skkk2d[:, 20, 0]], [skkk2d[:, 19, 1], skkk2d[:, 20, 1]],
                zs=[skkk2d[:, 19, 2], skkk2d[:, 20, 2]], c='red')
        ax.plot([skkk2d[:, 18, 0], skkk2d[:, 19, 0]], [skkk2d[:, 18, 1], skkk2d[:, 19, 1]],
                zs=[skkk2d[:, 18, 2], skkk2d[:, 19, 2]], c='red')
        ax.plot([skkk2d[:, 17, 0], skkk2d[:, 18, 0]], [skkk2d[:, 17, 1], skkk2d[:, 18, 1]],
                zs=[skkk2d[:, 17, 2], skkk2d[:, 18, 2]], c='red')
        plt.savefig('%s/%d.png' % ('/data/cyj/etri/result/plt',ix))
        plt.clf()
