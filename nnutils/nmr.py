...
import neural_renderer as neural_renderer

# from ..nnutils import geom_utils
import nnutils.geom_utils
...
class NMR(object):
    ...
    def forward_depth(self, vertices, faces):
        ...
        self.masks = self.renderer.render_depth(self.vertices, self.faces)

        masks = self.masks.data.get()
        return masks

    def backward_depth(self, grad_masks):
        ''' Compute gradient of vertices given mask gradients.
        Args:
            grad_masks: B X 256 X 256 numpy array
        Returns:
...
class Render(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, vertices, faces, renderer):
        # B x N x 3
...
class Render2(torch.autograd.Function):
    ...
    def forward(self, vertices, faces, textures=None):
        ...
        if textures is None:
            self.mask_only = True
            masks = self.renderer.forward_depth(vs, fs)
            return convert_as(torch.Tensor(masks), vertices)
        else:
...
class NeuralRenderer(torch.nn.Module):
    ...
    def __init__(self, img_size=256):
        ...
        self.proj_fn = nnutils.geom_utils.orthographic_proj_withz
        self.offset_z = 0.
    ...
    # def forward(self, vertices, faces, s, t, textures=None):
    #     verts = vertices
...
class NeuralRenderer_depth(torch.nn.Module):
    """
    This is the core pytorch function to call.
    Every torch NMR has a chainer NMR.
    Only fwd/bwd once per iteration.
    ...
    def __init__(self, img_size=256):
        super(NeuralRenderer_depth, self).__init__()
        self.renderer = NMR()
        ...
        self.proj_fn = nnutils.geom_utils.orthographic_proj_withz
        #self.proj_fn = nnutils.geom_utils.orthographic_proj_withs
        self.offset_z = 0.
    ...
    def forward(self, vertices, faces, cams, textures=None):
        ...
        if textures is not None:
            return Render2(self.renderer)(verts, faces, textures)
        else:
            return Render2(self.renderer)(verts, faces)
    # def forward(self, vertices, faces, s, t, textures=None):
    #     verts = self.proj_fn(vertices, s, t, offset_z=self.offset_z)
    #
    #     if textures is not None:
    #         return Render2(self.renderer)(verts, faces, textures)
    #     else:
