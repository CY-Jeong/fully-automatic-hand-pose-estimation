...
def hinge_loss(loss, margin):
    ...
    zeros = torch.autograd.Variable(torch.zeros(1).cuda(device=DEVICENUM), requires_grad=False)
...
def entropy_loss(A):
    ...
    """
    entropy = -torch.sum(A * torch.log(A+ 0.0001), 1)
...
def kp_l2_loss(kp_pred, kp_gt):
    ...
    #vis = (torch.FloatTensor(kp_gt[:, :, 2, None]) > 0).float().cuda(device=DEVICENUM)

    # This always has to be (output, target), not (target, output)
    #return criterion(vis * kp_pred, vis * torch.FloatTensor(kp_gt[:, :, :2]).cuda(device=DEVICENUM))
    return criterion(kp_pred, torch.FloatTensor(kp_gt))
...
class EdgeLoss(object):
    ...
    def __init__(self, verts, edges2verts, margin=2, use_bad_edge=False, use_l2=False):
        ...
        self.zeros = torch.autograd.Variable(torch.zeros(1).cuda(device=DEVICENUM), requires_grad=False)
...
class LaplacianLoss(object):
    ...
    def __init__(self, faces):
        ...
        # V x V
        laplacian.faces = faces
        self.Lx = None

    def __call__(self, verts):
        self.Lx = laplacian.Laplacian.apply(verts)
        # Reshape to BV x 3
