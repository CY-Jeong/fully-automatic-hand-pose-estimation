...
def orthographic_proj_withz(X, cam, offset_z=0.):
    ...
    proj_z = proj[:, :, 2, None] + offset_z + trans[:, :, 2, None]

    return torch.cat((proj_xy, proj_z), 2)
def orthographic_proj_withs(X, s, t, offset_z=0.):
    """
    X: B x N x 3
    cam: B x 7: [sc, tx, ty, quaternions]
    Orth preserving the z.
    ...
    scale = s.contiguous().view(-1, 1, 1)
    trans = t.contiguous().view(t.size(0), 1, -1)
...
def orthographic_proj_withoutz(X, cam):
    """
    X: B x N x 3
    cam: B x 7: [sc, tx, ty, quaternions]
    Orth preserving the z.
